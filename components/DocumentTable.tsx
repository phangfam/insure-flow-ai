      ...
      {filtered.length === 0 ? (
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wide">
            <tr>
              {['File', 'Form Type', 'Life Assured', 'NRIC', 'Policy No', 'Agent', 'Status', 'Uploaded'].map(h => (
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(doc => (
              <tr key={doc.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-2 text-gray-600">{FORM_TYPE_LABELS[doc.form_type] ?? doc.form_type}</td>
